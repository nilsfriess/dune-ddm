...
class NonlinearPoissonFEM : public Dune::PDELab::NumericalJacobianVolume<NonlinearPoissonFEM<Param, FEM>>,
...
public:
  ...
  void lambda_volume(const EG& eg, const LFSV& lfsv, R& r) const
  {
    ...
    for (const auto& ip : rule) {
      ...
      for (size_t i = 0; i < lfsv.size(); i++) r.accumulate(lfsv, i, -f * phihat[i] * factor);
    }
  }
  ...
  void lambda_boundary(const IG& ig, const LFSV& lfsv, R& r) const
  {
    ...
    for (const auto& ip : rule) {
      ...
      for (size_t i = 0; i < lfsv.size(); i++) r.accumulate(lfsv, i, j * phihat[i] * factor);
    }
  }

  //! volume integral depending on test and ansatz functions
  template <typename EG, typename LFSU, typename X, typename LFSV, typename R>
  void alpha_volume(const EG& eg, const LFSU& lfsu, const X& x, const LFSV&, R& r) const
  {
    // types & dimension
    ...
    for (const auto& ip : rule) {
      ...
      for (size_t i = 0; i < lfsu.size(); i++) S.mv(gradphihat[i][0], gradphi[i][0]);

      // compute gradient of u
      Dune::FieldVector<RF, dim> gradu(0.0);
      for (size_t i = 0; i < lfsu.size(); i++) gradu.axpy(x(lfsu, i), gradphi[i][0]);

      // integrate (grad u)*grad phi_i + q(u)*phi_i
      auto factor = ip.weight() * geo.integrationElement(ip.position());
      auto q = param.q(u);
      for (size_t i = 0; i < lfsu.size(); i++) r.accumulate(lfsu, i, (gradu * gradphi[i][0] + q * phihat[i]) * factor);
    }
  }
};
