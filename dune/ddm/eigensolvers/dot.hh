  ...
  void dot(OtherBlockView V, DenseMatView R) const
  {
    ...
    for (std::size_t row=0; row < k - 7; row += 8) {
      for (std::size_t bcol=0; bcol < n; ++bcol) {
        ...
        for (std::size_t acol=0; acol < m; ++acol) {
          c_tmp[acol*n+bcol] +=
            this->data()[(row+0)*m + acol] * V.data()[(row+0)*n + bcol] +
            this->data()[(row+1)*m + acol] * V.data()[(row+1)*n + bcol] +
            this->data()[(row+2)*m + acol] * V.data()[(row+2)*n + bcol] +
            this->data()[(row+3)*m + acol] * V.data()[(row+3)*n + bcol] +
            this->data()[(row+4)*m + acol] * V.data()[(row+4)*n + bcol] +
            this->data()[(row+5)*m + acol] * V.data()[(row+5)*n + bcol] +
            this->data()[(row+6)*m + acol] * V.data()[(row+6)*n + bcol] +
            this->data()[(row+7)*m + acol] * V.data()[(row+7)*n + bcol];
        }
      }
    }
    #pragma omp critical
    ...
    for (std::size_t row = remaining_start; row < k; ++row) {
      for (std::size_t bcol=0; bcol < n; ++bcol) {
        #pragma omp simd
        for (std::size_t acol=0; acol < m; ++acol) {
          R(acol, bcol) += this->data()[row*m + acol] * V.data()[row*n + bcol];
        }
      }
    }
  }
