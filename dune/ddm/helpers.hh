...
std::vector<bool> detect_dirichlet_dofs(const Mat& A)
{
  ...
  for (auto ri = A.begin(); ri != A.end(); ++ri) {
    ...
    for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
      if ((ri.index() != ci.index() and *ci != 0.) or (ri.index() == ci.index() and *ci != 1.)) {
...
template <class Mat>
inline std::size_t max_row_size(const Mat& M)
{
  std::size_t max_size = 0;
  for (auto rIt = M.begin(); rIt != M.end(); ++rIt) max_size = std::max(max_size, static_cast<std::size_t>(rIt->size()));
  return max_size;
}
