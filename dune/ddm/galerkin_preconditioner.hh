  ...
  struct CopyGatherScatterWithRank {
    ...
    static void scatter(VecDistributor& vd, const DataType& data, std::size_t i)
    {
      ...
      if (vd.others.count(rank) == 0) {
        logger::error_all("CopyGatherScatterWithRank: Rank {} is no neighbour", rank);
...
public:
  ...
  void apply(Vec& x, const Vec& d) override
  {
    ...
    Vec d0(total_num_t);
    MPI_Gatherv(d_local.data(), static_cast<int>(d_local.size()), MPI_DOUBLE, d0.data(), num_t_per_rank.data(), offset_per_rank.data(), MPI_DOUBLE, 0, mpicomm);
...
private:
  ...
  template <class Mat>
  void build_solver(const Mat& A, const Dune::ParameterTree& ptree, const std::string& subtree_name)
  {
    Logger::ScopedLog se(build_solver_event);
    auto mpicomm = comm->communicator();
    int rank = comm->communicator().rank();
    int size = comm->communicator().size();
    ...
    for (auto nid : comm->remoteIndices().getNeighbours()) neighbours[nid] = true;

    std::vector<bool> owner_mask(restr_vecs[0].N(), false);
    for (std::size_t l = 0; l < owner_mask.size(); ++l) owner_mask[l] = glis.pair(l)->local().attribute() == Dune::OwnerOverlapCopyAttributeSet::owner;
    ...
    Vec zerovec(s);
    zerovec = 0;
    std::vector<int> neighbour_vec(comm->remoteIndices().getNeighbours().begin(), comm->remoteIndices().getNeighbours().end());
