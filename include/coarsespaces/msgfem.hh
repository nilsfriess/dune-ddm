...
std::vector<Dune::BlockVector<Dune::FieldVector<double, 1>>> buildMsGFEMCoarseSpace(const RemoteIndices &ovlp_ids, const Mat &Aovlp, const std::vector<TripleWithRank> &remote_ncorr_triples,
                                                                                    const std::vector<TripleWithRank> &own_ncorr_triples, const std::vector<TripleWithRank> &own_ncorr_triples2,
                                                                                    const std::vector<bool> &interior_dof_mask, const Vec &dirichlet_mask_novlp, const Vec &pou,
                                                                                    const Dune::ParameterTree &ptree, std::vector<bool> &bnnz, std::vector<bool> &annz, std::vector<bool> &a_int_nnz)
{
  ...
  if (msgfem_type == "standard") {
    ...
    for (auto ri = B.begin(); ri != B.end(); ++ri) {
      if (ri.index() >= num_interior + num_boundary) {
        continue;
      }

      for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
        if (*ci > 0) {
          bnnz[reverse_ordering[ri.index()]] = true;
        }
      }
    }
    ...
    for (auto ri = A.begin(); ri != A.end(); ++ri) {
      ...
      for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
        if (*ci > 0) {
          annz[reverse_ordering[ri.index()]] = true;
        }
      }
    }
    ...
    for (auto ri = B.begin(); ri != B.end(); ++ri) {
      ...
      for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
        if (*ci > 0) {
          bnnz[ring_to_subdomain[reverse_ordering[ri.index()]]] = true;
        }
      }
    }
    ...
    for (auto ri = A.begin(); ri != A.end(); ++ri) {
      ...
      for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
        if (*ci > 0) {
          annz[ring_to_subdomain[reverse_ordering[ri.index()]]] = true;
        }
      }
    }
    ...
    a_int_nnz.resize(Aovlp.N(), false);
    for (std::size_t i = 0; i < interior_dof_mask.size(); ++i) {
      if (boundary_dst[i] > inner_boundary_dist) {
        a_int_nnz[i] = true;
      }
