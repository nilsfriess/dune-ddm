...
buildGenEOCoarseSpace(const RemoteIndices &ovlp_ids, const Mat &Aovlp, const std::vector<TripleWithRank> &remote_ncorr_triples, const std::vector<TripleWithRank> &own_ncorr_triples,
                      const std::vector<TripleWithRank> &own_ncorr_triples2, const std::vector<bool> &interior_dof_mask, const Vec &dirichlet_mask_novlp, const Vec &pou,
                      const Dune::ParameterTree &ptree, std::set<std::size_t> &ring_boundary, std::vector<bool> &bnnz, std::vector<bool> &annz)
{
  ...
  // dofs on the interior ring boundary (using overlapping subdomain numbering)
  ...
  if (geneo_type == "full" or geneo_type == "overlap") {
    ...
    for (auto ri = B.begin(); ri != B.end(); ++ri) {
      for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
        if (false or (ring_boundary.contains(ring_to_subdomain[ri.index()]) or ring_boundary.contains(ring_to_subdomain[ci.index()]))) {
          *ci = 0;
        }
        else {
    ...
    bnnz.resize(Aovlp.N(), false);
    for (auto ri = B.begin(); ri != B.end(); ++ri) {
      for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
        if (*ci > 0) {
          bnnz[ring_to_subdomain[ri.index()]] = true;
        }
      }
    }

    annz.resize(Aovlp.N(), false);
    for (auto ri = A.begin(); ri != A.end(); ++ri) {
      for (auto ci = ri->begin(); ci != ri->end(); ++ci) {
        if (*ci > 0) {
          annz[ring_to_subdomain[ri.index()]] = true;
        }
      }
    }
  }
