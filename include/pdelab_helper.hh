...
/**
 * @brief Creates remote indices for parallel domain decomposition communication.
 *
 * This function sets up the parallel communication infrastructure needed for
 * domain decomposition methods by creating a globally consistent indexing scheme
 * for degrees of freedom (DOFs) across multiple MPI processes.
 *
 * The function performs the following key operations:
 * 1. Generates globally unique DOF indices by combining local indices with MPI rank
 * 2. Resolves ownership conflicts at process boundaries using minimum consensus
 * 3. Identifies shared DOFs between neighboring processes
 * 4. Builds communication patterns for parallel sparse matrix operations
 *
 * @tparam GFS Grid function space type providing the DOF layout
 * @param gfs The grid function space containing the DOF structure
 * @param helper MPI helper object providing rank and communicator information
 *
 * @return A remote parallel indices object that can be used for parallel
 *         communication operations, particularly in iterative solvers and
 *         domain decomposition preconditioners
 *
 * @note This must be called collectively on all ranks in the communicator
 *       of the \p helper object.
 */
template <class GFS>
auto create_remote_indices(const GFS &gfs, const Dune::MPIHelper &helper)
{
